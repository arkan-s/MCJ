...
interface CPTable<TData, TValue> {
    ...
    data: TData[]
    addFn: (data: {namaPosition: string, dept: string }) => void
}
...
export function PositionDataTable<TData, TValue>({columns,data, addFn}: CPTable<TData, TValue>){
    ...
    const handleSave = () => {
        ...
        if (!newPosition || !dept) {
            alert("Isilah nama posisi yang baru dan pilih departmentnya!");
            return;
        }

        const newData = {
            namaPosition: newPosition,
            dept: dept,
        };
    ...
    return (
        <div>
            ...
            {positionTable ? (
                <Table>
                    <TableHeader>
                    {positionTable.getHeaderGroups()?.length ? (
                        positionTable.getHeaderGroups().map((headerGroup) => (
                    ...
                    <TableBody>
                    {positionTable.getRowModel().rows?.length ? (
                        positionTable.getRowModel().rows.map((row) => (
                        <TableRow
                            key={row.id}
            ...
            <div className="flex items-center justify-end space-x-2 py-4">
                ...
                variant="outline"
                size="sm"
                onClick={() => positionTable.previousPage()}
                disabled={!positionTable.getCanPreviousPage()}
                >
                Previous
                </Button>

                {positionTable.getPageCount() > 1 && (() => {
                    const pageIndex = positionTable.getState().pagination.pageIndex;
                    const totalPages = positionTable.getPageCount();
                        ...
                        <>
                            {Array.from({ length: end - start + 1 }, (_, i) => start + i).map((page) => (
                                <Button
                                    ...
                                    size="sm"
                                    onClick={() => positionTable.setPageIndex(page)}
                                >
                                    {page + 1}
                                </Button>
                            ))}
                ...
                variant="outline"
                size="sm"
                onClick={() => positionTable.nextPage()}
                disabled={!positionTable.getCanNextPage()}
                >
                Next
                </Button>
            </div>
