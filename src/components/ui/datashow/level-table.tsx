...
export function LevelDataTable<TData, TValue>({columns,data}: CPTable<TData, TValue>){
    ...
    return (
        <div>
            ...
            {levelTable ? (
                <Table>
                    <TableHeader>
                    {levelTable.getHeaderGroups()?.length ? (
                        levelTable.getHeaderGroups().map((headerGroup) => (
                    ...
                    <TableBody>
                    {levelTable.getRowModel().rows?.length ? (
            ...
            <div className="flex items-center justify-end space-x-2 py-4">
                ...
                variant="outline"
                size="sm"
                onClick={() => levelTable.previousPage()}
                disabled={!levelTable.getCanPreviousPage()}
                >
                Previous
                </Button>

                {levelTable.getPageCount() > 1 && (() => {
                    const pageIndex = levelTable.getState().pagination.pageIndex;
                    const totalPages = levelTable.getPageCount();
                        ...
                        <>
                            {Array.from({ length: end - start + 1 }, (_, i) => start + i).map((page) => (
                                <Button
                                    ...
                                    size="sm"
                                    onClick={() => levelTable.setPageIndex(page)}
                                >
                                    {page + 1}
                                </Button>
                            ))}
                ...
                variant="outline"
                size="sm"
                onClick={() => levelTable.nextPage()}
                disabled={!levelTable.getCanNextPage()}
                >
                Next
                </Button>
            </div>
