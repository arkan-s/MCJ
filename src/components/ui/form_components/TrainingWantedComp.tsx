...
export function TrainingWantedComponents(){
    function TrainingWantedComponent({indexComp}:{indexComp:number}){
        return (
            <div className={`${bitter.className} block flex-col gap-5 shadow-lg p-3 w-auto h-auto bg-white rounded-lg`}>
                <div className="flex flex-wrap justify-between gap-1">
                    <div className="flex flex-col gap-1 w-full">
                        <label htmlFor={`name${String(indexComp)}`}><b>Nama Training</b></label>
                        <input
                            type="text"
    ...
    const handleAddComponent = () => {
        setTrainingWanted(trainingWanted === null ?
                                [""] : 
                                [...trainingWanted, ""]
                        )
    }

    const handleOnBlur = (valString: string = "")=>{
        const newTrainingWanted = [...(trainingWanted || []), valString];
        
        return setTrainingWanted(newTrainingWanted)
    }
    ...
    return (
        ...
        {/* MAP CONTEXT DI SINI */}
        <form>
            {trainingWanted?.map((e, indexcomp)=>{
                ...
                return(
                    <motion.div
                        ...
                        className="mb-2"
                    >
                        <TrainingWantedComponent indexComp={indexcomp}></TrainingWantedComponent>
                    </motion.div>
                )
            })}
        </form>
