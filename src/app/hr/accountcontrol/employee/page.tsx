...
async function fetchData() {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/dataemployeebulk`, {
            method: 'GET',
            headers: {
...
export default async function accountcontrolEmployee(){
    ...
    return (
        <div className="container mx-auto pt-1">
            <DataTable columns={columns} data={data} addDataURL={json_data.status === 404 ? "/../addaccount/employeexcel":"/../addaccount/employee"} />
        </div>
    )
